...
StringHandler::StringHandler(std::string& target) : _ref_string(target) {
}
...
size_t StringHandler::count(char c) const {
    ...
    for (std::string::const_iterator cit = _ref_string.begin();
         cit != _ref_string.end(); ++cit) {
        if (*cit == c) {
            ++count;
        }
    }
