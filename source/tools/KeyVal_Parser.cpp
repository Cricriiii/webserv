...
static bool _common_values(const char* equality_set,
                           const char* separator_set) {
    while (*equality_set) {
        if (_in_set(*equality_set++, separator_set))
            return true;
    }
    return false;
}

__attribute__((nonnull(2, 3))) std::map<std::string, std::string>
KeyVal_Parser::build_dictionary(const std::string& ref_string,
                                const char* equality, const char* separator,
                                match_mode equ_mode, match_mode sep_mode) {
    using namespace std;

    if ((equ_mode == CHARSET || sep_mode == CHARSET) &&
        _common_values(equality, separator)) {
        throw logic_error(
            "KeyVal_Parser::build_dictionary: Common values in equality and "
            "separator");
    }
