...
void	hexdump(const char* c_string)
{
	...
	while (*c_string) {
		...
		oss	<< std::setw(LINE_NUM_WIDTH) << std::setfill('0') << std::dec << line_count << "  ";

		size_t	i = 0;

		for ( ; i < HEXDUMP_WIDTH / CHAR_WIDTH; ++i) {
			if (*c_string) {
				oss	<< std::setw(CHAR_WIDTH) << std::setfill('0') << std::hex << static_cast<unsigned char>(*c_string++) << SP;
			}
			else {
...
void	hexdump(const char* c_string, size_t size)
{
	...
	for (size_t n = 0; n < size; n += HEXDUMP_WIDTH) {
		...
		for ( ; i < HEXDUMP_WIDTH / 2; ++i) {
			if (p < end)
				oss << std::setw(CHAR_WIDTH) << std::setfill('0') << std::hex << static_cast<int>(static_cast<unsigned char>(*p++)) << SP;
			else
				oss << std::setw(CHAR_WIDTH + 1) << SP;
		}
