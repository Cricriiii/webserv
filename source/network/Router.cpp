...
Router::Router(std::vector<VServer>& vs) : _vservers(vs) {
}
...
static const std::string& _get_host_str(const HTTP_Request::header_t& headers) {
    ...
    if (host_head == headers.end()) {
        throw std::runtime_error(
            "Router: Should never reach router.route() without host header");
    }
...
int Router::_getsock_proto_info(int socketd, int* port, void* ip_bin,
                                int* proto_family) const {
    struct sockaddr_storage addr_store = {};
    socklen_t len = sizeof(addr_store);

    if (getsockname(socketd, reinterpret_cast<struct sockaddr*>(&addr_store),
                    &len) == -1) {
        return -1;
    }
    ...
    *proto_family = addr_store.ss_family;
    ...
    if (addr_store.ss_family == AF_INET) {
        struct sockaddr_in* sin =
            reinterpret_cast<struct sockaddr_in*>(&addr_store);
        *port = ntohs(sin->sin_port);
        ...
        struct sockaddr_in6* sin6 =
            reinterpret_cast<struct sockaddr_in6*>(&addr_store);
...
request_context_t Router::route(HTTP_Request& request) {
    ...
    if (_getsock_proto_info(request.socketd, &port, buffer_ip, &proto_family) ==
        -1) {
    ...
    if (!HTTP_IS_ERR(compliance.status)) {
        if (compliance.port != -1 && port != compliance.port) {
            ...
            for (vserv_it_t it = _vservers.begin(); it != _vservers.end();
                 ++it) {
