...
NetworkManager::NetworkManager(vserver_list& vserv, const http_lport_list& lports, const conf_http_t& http_conf)
...
{
	try {
		for (http_lport_list::iterator it = lports.begin(); it != lports.end(); ++it) {
			_new_acceptor(*it, PROTO_IPV4);
			_new_acceptor(*it, PROTO_IPV6);
		}

		_allocate_buffer(_ibuffer, define_size(_DFLT_RECV_SEGMENT_SIZ, _http_conf.recv_segment_siz));
		_allocate_buffer(_obuffer, define_size(_DFLT_SEND_SEGMENT_SIZ, _http_conf.send_segment_siz));
	}
...
NetworkManager::~NetworkManager()
{
	...
	for (http_conn_list::iterator it = _connections.begin(); it != _connections.end(); ++it) {
...
void	NetworkManager::_conn_read(NetConnection& cur_conn)
{
	if (cur_conn.wait_before_next_io()) {
		...
		else if ( ( _ibuffer.size = recv(cur_conn.socketd(), _ibuffer.data, _ibuffer.capacity, 0)) > 0) {
...
bool	NetworkManager::_cgi_kill_if_timeout(NetConnection& cur_conn, long long timeout_time)
{
...
redispatch:
	switch (cgi_state) {
		...
		case CGI_WAIT_ERR:
		case CGI_EXEC_FAIL:
			cur_conn.set_server_err(HTTP_IS_ERR(http_status) ? http_status : E_HTTP_INTERNAL_SERVER_ERROR);
			cur_conn.set_state(S_CONN_HANDLE);
			break;
	}
}
...
{
	_mod_client_interest_set(cur_conn.socketd(), SCAN_EVENT_OUT);
	if (cur_conn.read_cgi_output(_ibuffer.data, &_ibuffer.size, _ibuffer.capacity)) {
...
void	NetworkManager::_conn_dload_body(NetConnection& cur_conn)
{
	if (cur_conn.wait_before_next_io()) {
		...
		_ibuffer.size = recv(cur_conn.socketd(), _ibuffer.data, _ibuffer.capacity, 0);
...
{
	_mod_client_interest_set(cur_conn.socketd(), SCAN_EVENT_IN | SCAN_EVENT_OUT);
...
{
	for (http_rconn_list::iterator it = _ready_lst.begin(); it != _ready_lst.end(); ++it) {
		...
		while (1) {
			...
			switch (cur_conn.state()) {
				...
				case S_CONN_WAIT:			_ready_del_lst.push_back(*it);	break;
				
				case S_CONN_READ:			_conn_read(cur_conn);			break;
				case S_CONN_HANDLE:			_conn_handle(cur_conn);			break;

				case S_CONN_WRITE_TO_CGI:	_conn_write_to_cgi(cur_conn);	break;
				case S_CONN_CGI_WAIT:		_conn_cgi_wait(cur_conn);		break;
				case S_CONN_DLOAD_CGI:		_conn_dload_cgi(cur_conn);		break;
				case S_CONN_DLOAD_BODY:		_conn_dload_body(cur_conn);		break;
				case S_CONN_BUILD_HEAD:		_conn_build_head(cur_conn);		break;

				case S_CONN_SEND_HEAD:		_conn_send_head(cur_conn);		break;
				case S_CONN_SEND_BODY:		_conn_send_body(cur_conn);		break;

				case S_CONN_CLI_CLOSE:		_close_lst.push_back(*it);		break;
			}

			if (cur_conn.state_change() == false) break;
		}
	}
...
nm_send_state_t	NetworkManager::_send_headers(NetConnection& cur_conn, int socketd)
{
	response_status_t	status = cur_conn.head_stream(_obuffer.data, _obuffer.capacity);
	
	_obuffer.size = send(socketd, _obuffer.data, status.cur, 0);

	if (ENABLE_IO_HEXDUMP) hexdump(_obuffer.data, _obuffer.size);
...
nm_send_state_t	NetworkManager::_send_body(NetConnection& cur_conn, int socketd)
{
	response_status_t	status = cur_conn.body_stream(_obuffer.data, _obuffer.capacity);
	_obuffer.size = send(socketd, _obuffer.data, status.cur, 0);
...
{
	for (http_rconn_list::iterator it = _ready_del_lst.begin(); it != _ready_del_lst.end(); ++it) {
		_connections.find(*it)->second.set_readiness(false);
		_ready_lst.erase(std::remove(_ready_lst.begin(), _ready_lst.end(), *it), _ready_lst.end());
	}

	for (http_rconn_list::iterator it = _close_lst.begin(); it != _close_lst.end(); ++it) {
		_ready_lst.erase(std::remove(_ready_lst.begin(), _ready_lst.end(), *it), _ready_lst.end());
