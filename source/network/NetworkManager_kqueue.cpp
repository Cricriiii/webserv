...
static void _kqueue_add_socket(int epfd, int socket) {
    int yes = 1;
    if (setsockopt(socket, SOL_SOCKET, SO_NOSIGPIPE, &yes, sizeof(yes)) == -1)
        throw std::runtime_error("setsockopt: SO_NOSIGPIPE");
    ...
    if (kevent(epfd, kev, 2, NULL, 0, NULL) == -1)
...
static void _kqueue_mod_socket(int epfd, int socket, int event, int status) {
    ...
    if (kevent(epfd, &kev, 1, NULL, 0, NULL) == -1)
        throw std::runtime_error("kevent: modif");
}
...
void NetworkManager::_init_event_polling() {
    ...
    for (acceptor_list::iterator it = _acceptors.begin();
        ...
        if (kevent(_event_fd, &kev, 1, NULL, 0, NULL) == -1)
            throw std::runtime_error("kevent: signal");
    }
}
...
void NetworkManager::listen() {
    ...
    if (nb_events == -1)
        throw std::runtime_error("kevent: run");
        ...
        if (_acceptors.find(sd) != _acceptors.end()) {
            while (1) {
                ...
                _connections.insert(std::make_pair(
                    accepted,
                    NetConnection(accepted, _http_conf, _obuffer.capacity)));
...
void NetworkManager::_mod_client_interest_set(int socketd,
                                              poll_op_t new_state) {
