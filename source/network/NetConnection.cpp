...
NetConnection::NetConnection(int socketd, const conf_http_t& http_conf, size_t obuf_siz)
	:	_socketd(socketd),
...
bool	NetConnection::_retrieve_session()
{
	...
	if (sid_cookie != _request.cookies.end()) {
		SessionManager::Session*	session = SessionManager::get_instance().get_session(sid_cookie->second);
...
{
	if (ENABLE_IO_HEXDUMP) hexdump(buffer, size);
...
void	NetConnection::_define_close_behavior()
{
	if (HTTP_IS_ERR(_request.status)) {
		...
		HTTP_Request::header_it_t	connection = _request.headers.find("connection");
...
void	NetConnection::feed_cgi()
{
	...
	if (_request.body_transfer_typ == LENGTH_TRANSF) {
		...
		written = write(_resource.file_cgi_write.fd, 
							buf.data + _total_payload_progress, 
		...
		if (_total_payload_progress >= _resource.file_cgi_write.size) {
			...
			close(_resource.file_cgi_write.fd), _resource.file_cgi_write.fd = -1;
		}
...
void	NetConnection::_flush_body_into_file()
{
	...
	if (_request.body_transfer_typ == LENGTH_TRANSF) {
		...
		size_t	to_copy = static_cast<ssize_t>(_resource.file_write.size) > buf.size ? buf.size : _resource.file_write.size;

		_total_payload_progress = 0;
		written = write(_resource.file_write.fd, 
							buf.data + _total_payload_progress, 
...
void	NetConnection::_flush_post_header_buffer()
{
	...
	switch (_request.body_transfer_typ) {
		...
		case EMPTY_TRANSF:
			throw std::runtime_error("NetConnection: Unexpected state in handle_request write path");
	}
}
...
bool	NetConnection::_save_content_body(char* buffer, ssize_t* buffer_size)
{
	...
	if (*buffer_size > 0) {
		size_t	to_copy;
		if (_total_payload_progress + *buffer_size <= _resource.file_write.size) {
			to_copy = *buffer_size;
		}
		else {
...
bool	NetConnection::_save_chunk_body(char* buffer, ssize_t* buffer_size)
{
	...
	if (*buffer_size > 0) {
		...
		while (payload_progress < static_cast<ssize_t>(_request.entity_body.data.size())) {
			payload_progress = write(_resource.file_write.fd,
										_request.entity_body.data.c_str() + payload_progress,
										to_copy);
...
bool	NetConnection::save_body(char* buffer, ssize_t* buffer_size)
{
	switch (_request.body_transfer_typ) {
		...
		case LENGTH_TRANSF:		return _save_content_body(buffer, buffer_size);
		case CHUNKED_TRANSF:	return _save_chunk_body(buffer, buffer_size);
		case EMPTY_TRANSF:		return true;
		default:				return _request.status = E_HTTP_NOT_IMPLEMENTED, true;		
	}
}
...
void	NetConnection::_add_header_if_length()
{
	...
	if (_resource.file_cgi_read.fd != -1) {
		...
		HTTP_Request::header_cit_t	cit = char_tools::map_string_find_nocase(_response.headers, "content-length");
...
void	NetConnection::_create_header(const char* key, const char* val)
{
...
__attribute__((nonnull))
void	NetConnection::_append_header(const char* key, const char* val)
{
...
void	NetConnection::build_headers()
{
	...
	_create_header("Connection", _keep_alive && !_kill ? "keep-alive" : "close");

	_add_header_if_session();
	_add_header_if_length();

	if (!_resource.mime_type.empty()) _create_header("Content-type", _resource.mime_type.c_str());
}
...
int		NetConnection::kill_cgi()
{
	...
	if (_resource.cgi.process != -1) {
		...
		if (_resource.file_cgi_read.fd != -1) ::close(_resource.file_cgi_read.fd);
		if (_resource.file_cgi_write.fd != -1) ::close(_resource.file_cgi_write.fd);
...
{
	HTTP_Request::header_t::iterator	it_login = char_tools::map_string_find_nocase(header_list, "New-Login");
	...
	if (it_login != header_list.end()) {
		...
		if (!SessionManager::get_instance().retrieve_login(it_login->second)) {
			...
			cur_session->values.insert(std::make_pair("login", it_login->second));
		}
	}
}
...
void	NetConnection::_parse_cgi_headers()
{
	...
	if (headers.size()) {
		...
		HTTP_Request::header_t header_list = KeyVal_Parser::build_dictionary(headers, ":", "\r\n", CHARSET, LITERAL);

		_try_new_session(header_list);

		for (std::map<std::string, std::string>::iterator it = header_list.begin(); it != header_list.end(); ++it) {
			_response.headers.insert(std::make_pair(it->first, it->second));
		}
		
		size_t	offset = pos + 4;	/*	pos + \r\n\r\n	*/
		_response.entity_body.data.erase(_response.entity_body.data.begin(),
		_response.entity_body.data.begin() + offset);
	}
}


bool	NetConnection::read_cgi_output(char* buffer, ssize_t* buffer_read, size_t buffer_capacity)
{
	if (wait_before_next_io()) {
		...
		if ( (*buffer_read = read(_resource.file_cgi_read.fd, buffer, buffer_capacity)) > 0) {
