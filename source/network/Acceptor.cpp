...
Acceptor::Acceptor(int port, protocol_type protocol)
    : _socketd(-1), _port(port) {
    try {
        if (port < PORT_MIN_VALUE || port > PORT_MAX_VALUE)
            throw std::invalid_argument(
                "Acceptor: invalid port in server configuration");
        ...
        switch (protocol) {
            case PROTO_IPV4:
                ...
                if ((_socketd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
                    throw std::runtime_error(
                        "Acceptor: socket creation failed");
                ...
                if (setsockopt(_socketd, SOL_SOCKET, SO_REUSEADDR, &yes,
                               sizeof(yes)) == -1) {
                    throw std::runtime_error("setsockopt: SO_REUSEADDR IPV4");
                }

                if (bind(_socketd, (struct sockaddr *)&addr,
            ...
            case PROTO_IPV6:
                ...
                if (setsockopt(_socketd, IPPROTO_IPV6, IPV6_V6ONLY, &yes,
                               sizeof(yes)) == -1) {
