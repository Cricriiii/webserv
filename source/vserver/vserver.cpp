...
{
	for (std::map<int, std::string>::const_iterator it = _vserver.errors.begin(); it != _vserver.errors.end(); ++it)
	{
		_context.errors.insert(std::make_pair(it->first, _vserver.root + it->second));
	}
	...
	if (!location)
	{
		_context.resolved_path = _vserver.root + _context.request->start_line.URL.substr(0, _context.request->start_line.URL.find('?'));
	...
	else
	{
		...
		{
			_context.resolved_path = location->root + _context.request->start_line.URL.substr(0, _context.request->start_line.URL.find('?'));
...
{
	if (_context.request->entity_body.size > static_cast<size_t>(_vserver.max_body_size))
	{
...
{
	for (std::set<std::string>::const_iterator it = _vserver.name.begin(); it != _vserver.name.end(); ++it)
	{
...
void	VServer::print_context() const
{
	...
	for (std::map<int, std::string>::const_iterator it = _context.errors.begin(); it != _context.errors.end(); ++it)
		std::cout << "  " << it->first << ": " << it->second << std::endl;
	std::cout << "limit_except: ";
	for (std::set<std::string>::const_iterator it = _context.limit_except.begin(); it != _context.limit_except.end(); ++it)
		std::cout << *it << " ";
	std::cout << std::endl;
	std::cout << "file_succes_download: " << _context.file_succes_download << std::endl;
	std::cout << "file_succes_delete: " << _context.file_succes_delete << std::endl;
	std::cout << "file_tmp_css: " << _context.file_tmp_css << std::endl;
	std::cout << "server_name: ";
	for (std::set<std::string>::const_iterator it = _context.server_name.begin(); it != _context.server_name.end(); ++it)
		std::cout << *it << " ";
	std::cout << std::endl;
	std::cout << "server_port: " << _context.server_port << std::endl;
	std::cout << "root: " << _context.root << std::endl;
	std::cout << "index: ";
	for (std::vector<std::string>::const_iterator it = _context.index.begin(); it != _context.index.end(); ++it)
		std::cout << *it << " ";
	std::cout << std::endl;
}
