...
resource_context_t	AutoIndexHandler::execute(request_context_t& context)
{
	...
	body_auto +=	"<!DOCTYPE html>\n"
					...
					"  <div class=\"container\">\n"
					"    <h1>Index of " + uri + "</h1>\n"
					"    <p class=\"path\">Directory requested : " + uri + "</p>\n";

	if (context.limit_except.size() == 0 || context.limit_except.find("PUT") == context.limit_except.end())
		body_auto +=	"    <input type=\"file\" id=\"upload-file\">\n"
	...
	while ((entry = readdir(dir)) != NULL)
	{
		...
		{
			href += "/";
			body_auto += "      <li class=\"directory\"><a href=\"" + href + "\">" + name + "/</a></li>\n";
		}
		else
		{
			body_auto += "      <li class=\"file\"><a href=\"" + href + "\">" + name + "</a>\n";

			if (context.limit_except.size() == 0 || context.limit_except.find("DELETE") == context.limit_except.end())
				body_auto += "      <button class=\"delete-btn\" data-path=\"" + href + "\">Delete</button>\n";
	...
	resource.file_tmp.fd = open(autoindex_file.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0644);
	...
	{
		ssize_t bits_send = write(resource.file_tmp.fd, body_auto.c_str() + n_bits, body_auto.size() - n_bits);
		if (bits_send <= 0)
		{
