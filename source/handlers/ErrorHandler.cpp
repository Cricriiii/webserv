...
ErrorHandler::ErrorHandler(int code) : _code(code) {
}
...
static void creation_error_file(request_context_t& context,
                                resource_context_t& resource, int code) {
    ...
    error_page +=
        ...
        "</h1>\n"
        "      <p class=\"message\">" +
        const_cast<char*>(http_error_reason(code)) +
        "</p>\n"
    ...
    resource.file_tmp.fd =
        open(name_page.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0644);
    if (resource.file_tmp.fd != -1) {
        size_t n_bits = 0;
        while (n_bits < error_page.size()) {
            ssize_t bits_send =
                write(resource.file_tmp.fd, error_page.c_str() + n_bits,
                      error_page.size() - n_bits);
            if (bits_send <= 0) {
...
resource_context_t ErrorHandler::execute(request_context_t& context) {
    ...
    if (context.errors.find(_code) != context.errors.end()) {
        resource.file_read.fd =
            open(context.errors.find(_code)->second.c_str(), O_RDONLY);

        if (resource.file_read.fd != -1) {
            resource.mime_type =
                get_mime_type(context.errors.find(_code)->second);
            struct stat status = {};
