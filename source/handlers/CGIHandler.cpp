...
static std::vector<char*> build_env(request_context_t& context) {
    ...
    env.push_back(
        "SERVER_PROTOCOL=" + context.request->start_line.version.name + '/' +
        char_tools::itostr(context.request->start_line.version.minor) + '.' +
        char_tools::itostr(context.request->start_line.version.major));
    ...
    if (context.request->entity_body.size != 0) {
        env.push_back("CONTENT_TYPE=" +
                      context.request->headers.find("content-type")->second);
        env.push_back("CONTENT_LENGTH=" +
                      char_tools::itostr(context.request->entity_body.size));
    }

    std::map<std::string, std::string>::const_iterator cit =
        context.request->headers.begin();
    std::map<std::string, std::string>::const_iterator cite =
        context.request->headers.end();
...

    {
        if (context.request->session) {
            SessionManager::session_data_t::iterator it =
                context.request->session->values.find("login");
...
static std::vector<char*> build_argv(request_context_t& context) {
    ...
    for (size_t i = 0; i < argv_construction.size(); ++i) {
        ...
        std::copy(
            argv_construction[i].c_str(),
            argv_construction[i].c_str() + argv_construction[i].size() + 1,
            tmp);
        argv.push_back(tmp);
    }
    argv.push_back(NULL);
...
static void process_child(request_context_t& context, int pipe_in[2],
                          int pipe_out[2]) {
