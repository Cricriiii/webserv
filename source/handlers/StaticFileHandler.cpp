...
resource_context_t StaticFileHandler::execute(request_context_t& context) {
    static ptr_fct ft_methods[5] = {
        &StaticFileHandler::get_method, &StaticFileHandler::post_method,
        &StaticFileHandler::delete_method, &StaticFileHandler::put_method};
...
resource_context_t StaticFileHandler::post_method(request_context_t& context) {
    ...
    HTTP_Request::header_it_t it =
        context.request->headers.find("content-type");
    ...
    resource.file_write.fd =
        open(filename.c_str(), O_CREAT | O_WRONLY | O_EXCL, 0644);
...
resource_context_t StaticFileHandler::delete_method(
...
resource_context_t StaticFileHandler::put_method(request_context_t& context) {
    ...
    resource.file_write.fd =
        open(context.resolved_path.c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0644);
