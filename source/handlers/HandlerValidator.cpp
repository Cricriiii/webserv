    ...
    if (context.directory) {
        ...
        for (size_t i = 0; i < context.index.size(); ++i) {
            ...
            if (access(complet_path.c_str(), F_OK) == 0) {
                if (access(complet_path.c_str(), R_OK) == -1 &&
                    !context.autoindex)
...
static IHandler* verif_method(request_context_t& context) {
    for (size_t i = 0; i < sizeof(methods) / sizeof(methods[0]); ++i) {
        if (context.request->start_line.method == methods[i]) {
            if (!context.limit_except.empty() &&
                context.limit_except.find(context.request->start_line.method) !=
...
static bool verif_cgi_extension(const std::string& path,
                                const std::string& cgi_extension) {
    if (path.size() < cgi_extension.size())
        return false;

    return (path.compare(path.size() - cgi_extension.size(),
                         cgi_extension.size(), cgi_extension) == 0);
}

static IHandler* verif_cgi(request_context_t& context) {
    if (context.cgi) {
        if (context.cgi_extension.empty() == false &&
            context.cgi_path.empty() == false) {
            ...
            if (verif_cgi_extension(context.resolved_path,
                                    context.cgi_extension) == false)
                return new ErrorHandler(E_HTTP_FORBIDDEN);
