...
void	Server_configuration::open_read_file(const char* path_file)
{
	...
	if (fd_conf == -1)		throw std::runtime_error (msg_error("open file", -1));

	while (1)
	{
		char	buffer[BUFFER_SIZE];
		ssize_t	octet = read(fd_conf, buffer, BUFFER_SIZE);
		if (octet == -1)	throw std::runtime_error (msg_error("read file", -1));
		if (octet == 0)		break;
	...
	close(fd_conf);
	if (_file.size() == 0)	throw std::runtime_error (msg_error("file is empty", -1));
}
...
void	Server_configuration::token_quote()
{
	...
	while (i < _file.size())
	{
		...
		{
			if (i + 1 >= _file.size())
				throw std::invalid_argument(msg_error("invalid escape at end of file", _line));
	...
	if (i + 1 < _file.size() && !space(_file[i + 1]) && !end_line(_file[i + 1]) && !directive(_file[i + 1]))
		throw std::invalid_argument(msg_error("invalid character after quote", _line));
...
void	Server_configuration::cut_word()
{
	...
	while (_i_file < _file.size() && !space(current_char()) && !directive(current_char()))
	{
		if (quote(current_char()))
			throw std::invalid_argument(msg_error("quote after character in word", _line));
		++_i_file;
	}
