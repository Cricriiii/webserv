...
void Server_configuration::parsing() {
    ...
    if (is_end_token(_i_token) || !has_status(_i_token, OPEN_BLOCK))
        throw std::invalid_argument(
            msg_error("expected '{' after 'http'", _i_token - 1));

    init_conf_http();

    ++_i_token;
    while (!is_end_token(_i_token) && !has_status(_i_token, CLOSE_BLOCK)) {
        if (is_token(_i_token, "client_http_max_body_siz") ||
            is_token(_i_token, "recv_segment_siz") ||
            is_token(_i_token, "http_max_header_amt_siz") ||
            is_token(_i_token, "send_segment_siz") ||
            is_token(_i_token, "http_max_method_siz") ||
            is_token(_i_token, "http_max_uri_siz") ||
            is_token(_i_token, "http_max_header_key_siz") ||
            is_token(_i_token, "http_max_header_val_siz") ||
            is_token(_i_token, "http_max_body_siz")) {
            parse_value_http();
        }

        else if (is_token(_i_token, "server")) {
            if (is_end_token(_i_token + 1) ||
                !has_status(_i_token + 1, OPEN_BLOCK))
                throw std::invalid_argument(
                    msg_error("expected '{' after 'server'", _i_token));
    ...
    if (is_end_token(_i_token))
        throw std::invalid_argument(msg_error(
            "expected token '}' after end of http block", _i_token - 1));

    ++_i_token;
    if (_i_token != _tokens.size())
        throw std::invalid_argument(
            msg_error("unexpected token after end of http block", _i_token));
}
...
void Server_configuration::parsing_server() {
    ...
    if (is_end_token(_i_token))
        throw std::invalid_argument(
            msg_error("invalid structure for server block", _i_token - 1));

    while (!is_end_token(_i_token) && !has_status(_i_token, CLOSE_BLOCK)) {
        if (is_token(_i_token, "listen") || is_token(_i_token, "root") ||
            is_token(_i_token, "max_body_size"))
            parse_single_value_directive(server);

        else if (is_token(_i_token, "index") ||
                 is_token(_i_token, "server_name"))
    ...
    if (is_end_token(_i_token))
        throw std::invalid_argument(
            msg_error("expected '}' after server block", _i_token - 1));

    if (verif_server_conflict(server) == true)
        throw std::invalid_argument(
            msg_error("conflict server_name and port", _i_token));
...
void Server_configuration::parse_value_http() {
    ...
    if (is_end_token(_i_token) || !has_status(_i_token, SEMICOLON))
        throw std::invalid_argument(
            msg_error("expected ';' after simple guidelines", start));

    else if (is_token(start, "recv_segment_siz")) {
        if (_conf_http.recv_segment_siz != -1)
            throw std::invalid_argument(
                msg_error("recv_segment_siz defined a second time", start));
        ...
        if (_conf_http.send_segment_siz != -1)
            throw std::invalid_argument(
                msg_error("send_segment_siz defined a second time", start));
        ...
        if (_conf_http.http_max_method_siz != -1)
            throw std::invalid_argument(
                msg_error("http_max_method_siz defined a second time", start));
        _conf_http.http_max_method_siz =
            strtoll(get_token(start + 1), start + 1);
    }

    else if (is_token(start, "http_max_uri_siz")) {
        if (_conf_http.http_max_uri_siz != -1)
            throw std::invalid_argument(
                msg_error("http_max_uri_siz defined a second time", start));
        ...
        if (_conf_http.http_max_header_key_siz != -1)
            throw std::invalid_argument(msg_error(
                "http_max_header_key_siz defined a second time", start));
        _conf_http.http_max_header_key_siz =
            strtoll(get_token(start + 1), start + 1);
    }
        ...
        if (_conf_http.http_max_header_val_siz != -1)
            throw std::invalid_argument(msg_error(
                "http_max_header_val_siz defined a second time", start));
        _conf_http.http_max_header_val_siz =
            strtoll(get_token(start + 1), start + 1);
    }
        ...
        if (_conf_http.http_max_header_amt_siz != -1)
            throw std::invalid_argument(msg_error(
                "http_max_header_amt_siz defined a second time", start));
        _conf_http.http_max_header_amt_siz =
            strtoll(get_token(start + 1), start + 1);
    }
        ...
        if (_conf_http.http_max_body_siz != -1)
            throw std::invalid_argument(
                msg_error("http_max_body_siz defined a second time", start));
...
void Server_configuration::parse_single_value_directive(vserver_t& server) {
    ...
    if (is_end_token(_i_token) || !has_status(_i_token, SEMICOLON))
        ...
        if (server.port != -1)
            throw std::invalid_argument(
                msg_error("listen defined a second time", start));
        ...
        if (server.max_body_size != -1)
            throw std::invalid_argument(
                msg_error("max_body_size defined a second time", start));
        ...
        if (server.root.empty() == false)
            throw std::invalid_argument(
                msg_error("root defined a second time", start));

        if (verif_path_root(get_token(start + 1)) == false)
            throw std::invalid_argument(
                msg_error("invalid root path", start + 1));
...
void Server_configuration::parse_multi_value_directive(vserver_t& server) {
    ...
    if (is_token(start, "index") && !server.index.empty())
        throw std::invalid_argument(
            msg_error("index defined a second time", start));

    if (is_token(start, "server_name") && !server.name.empty())
        throw std::invalid_argument(
            msg_error("server_name defined a second time", start));

    _i_token = start + 1;
    while (!is_end_token(_i_token)) {
        if (!has_status(_i_token, WORD) && !has_status(_i_token, SEMICOLON))
            throw std::invalid_argument(
                msg_error("expected ';' after simple guidelines", _i_token));
        ...
        if (is_token(start, "index"))
            ...
            if (server.name.insert(get_token(_i_token)).second == false)
                throw std::invalid_argument(msg_error(
                    "server_name defined a second time in the same server",
                    _i_token));
...
void Server_configuration::parse_error_page_directive(vserver_t& server) {
    ...
    while (!is_end_token(_i_token)) {
        ...
        if (error_codes.insert(strtoi(get_token(_i_token), _i_token)).second ==
            false)
            throw std::invalid_argument(msg_error(
                "error_page defined a second time for the same error code",
                _i_token));

        ++_i_token;
    }

    for (std::set<int>::const_iterator it = error_codes.begin();
...
void Server_configuration::parse_location_block(vserver_t& server) {
    ...
    if (is_end_token(start + 2))
        throw std::invalid_argument(
            msg_error("invalid structure for location", start));
    ...
    if (verif_location_path(server, get_token(start + 1)) == true)
        throw std::invalid_argument(msg_error(
            "location defined a second time for the same path", start + 1));

    if (verif_path(get_token(start + 1)) == false)
        throw std::invalid_argument(
            msg_error("invalid location path", start + 1));

    location.path = get_token(start + 1);

    if (!has_status(start + 2, OPEN_BLOCK))
        throw std::invalid_argument(
            msg_error("expected '{' after location path", start + 1));

    _i_token = start + 3;
    while (!is_end_token(_i_token) && !has_status(_i_token, CLOSE_BLOCK)) {
        if (is_token(_i_token, "root") || is_token(_i_token, "autoindex") ||
            is_token(_i_token, "cgi_extension") ||
            is_token(_i_token, "cgi_path") || is_token(_i_token, "cgi"))
            parse_single_value_directive_location(location);

        else if (is_token(_i_token, "limit_except") ||
                 is_token(_i_token, "index"))
            parse_multi_value_directive_location(location);

        else
            throw std::invalid_argument(
                msg_error("invalid token in location block", _i_token));
    }

    if (is_end_token(_i_token))
        throw std::invalid_argument(
            msg_error("unexpected end of file in location block", start));

    server.location.push_back(location);
    ++_i_token;
}

void Server_configuration::parse_single_value_directive_location(
    location_t& location) {
    size_t start = _i_token;
    ...
    if (is_token(start, "root")) {
        ...
        if (location.autoindex == true)
            throw std::invalid_argument(
                msg_error("autoindex defined a second time", start));
        ...
        if (location.cgi == true)
            throw std::invalid_argument(
                msg_error("cgi defined a second time", start));
        if (is_token(start + 1, "on"))
            location.cgi = true;
    }
        ...
        if (location.cgi_extension.empty() == false)
            throw std::invalid_argument(
                msg_error("cgi_extension defined a second time", start));

        if (get_token(start + 1).empty() || get_token(start + 1)[0] != '.')
            throw std::invalid_argument(
                msg_error("invalid cgi_extension", start + 1));
        ...
        if (location.cgi_path.empty() == false)
            throw std::invalid_argument(
                msg_error("cgi_path defined a second time", start));

        if (verif_path(get_token(start + 1)) == false)
            throw std::invalid_argument(
                msg_error("invalid cgi_path", start + 1));
...
void Server_configuration::parse_multi_value_directive_location(
    location_t& location) {
    size_t start = _i_token;
    ...
    if (is_token(start, "limit_except") && !location.limit_except.empty())
        throw std::invalid_argument(
            msg_error("limit_except defined a second time", start));
    ...
    while (!is_end_token(_i_token)) {
        ...
        if (is_token(start, "limit_except")) {
            if (location.limit_except.insert(get_token(_i_token)).second ==
                false)
                throw std::invalid_argument(msg_error(
                    "method defined a second time in the same location",
                    _i_token));
        }
