...
const std::string	Server_configuration::msg_error(const std::string& error, int index) const
{
	if (index < 0)
		return ".conf: " + error;

	return ".conf: " + error + " at line: " + char_tools::itostr(current_line(index));
}
...
int	Server_configuration::strtoi(const std::string& s, int index) const
{
	...
	if (out < 0)
		throw std::invalid_argument(msg_error("number must be positive", index));
...
bool	Server_configuration::verif_location_path(vserver_t& server, const std::string& path) const
{
...
bool	Server_configuration::is_token(size_t index, const std::string& value) const
{
...
bool	Server_configuration::has_status(size_t index, e_status_token status) const
{
