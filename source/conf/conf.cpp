...
Server_configuration::Server_configuration(const char* path_file)
    : _line(1),
      _file(),
      _i_file(0),
      _tokens(),
...
void Server_configuration::print_conf() const {
    ...
    for (size_t i = 0; i < _vservers.size(); i++) {
        ...
        for (std::map<int, std::string>::const_iterator it =
                 _vservers[i].errors.begin();
             it != _vservers[i].errors.end(); ++it)
            std::cout << "error_page: " << it->first
                      << " | path: " << it->second << std::endl;

        for (size_t k = 0; k < _vservers[i].location.size(); k++) {
            std::cout << std::endl;
            std::cout << "\tlocation path: " << _vservers[i].location[k].path
                      << std::endl;
            std::cout << "\tautoindex: "
                      << (_vservers[i].location[k].autoindex ? "on" : "off")
                      << std::endl;
            std::cout << "\troot: " << _vservers[i].location[k].root
                      << std::endl;

            for (size_t l = 0; l < _vservers[i].location[k].index.size(); l++)
                std::cout << "\tindex: " << _vservers[i].location[k].index[l]
                          << std::endl;

            for (std::set<std::string>::const_iterator it =
                     _vservers[i].location[k].limit_except.begin();
                 it != _vservers[i].location[k].limit_except.end(); ++it)
    ...
    for (std::set<int>::const_iterator it = _ports.begin(); it != _ports.end();
         ++it)
    ...
    std::cout << "\nhttp_max_body_siz: " << _conf_http.http_max_body_siz
              ...
              << "\nhttp_max_header_key_siz: "
              << _conf_http.http_max_header_key_siz
              << "\nhttp_max_header_val_siz: "
              << _conf_http.http_max_header_val_siz << std::endl;
