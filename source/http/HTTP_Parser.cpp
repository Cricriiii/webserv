...
#define GOTO(NEW_STATE)					SET_STATE(NEW_STATE); REDISPATCH
#define GOTO_ERR						SET_ERROR; REDISPATCH
#define REJECT_REQUEST(STATE)			request.status = (STATE); GOTO_ERR
#define IFN_ERR_SET_STATE(NEW_STATE)	if (IS_STATE(S_PARS_ERROR)) { REDISPATCH; } else { SET_STATE(NEW_STATE); }
#define IFN_ERR_GOTO(NEW_STATE)			if (IS_STATE(S_PARS_ERROR)) { REDISPATCH; } else { GOTO(NEW_STATE); }
...
{
	_http_max_method_siz = define_size(_DFLT_HTTP_MAX_METHOD_SIZ, http_conf.http_max_method_siz);
	_http_max_uri_siz = define_size(_DFLT_HTTP_MAX_URI_SIZ, http_conf.http_max_uri_siz);
	_http_max_header_key_siz = define_size(_DFLT_HTTP_MAX_HEADER_KEY_SIZ, http_conf.http_max_header_key_siz);
	_http_max_header_val_siz = define_size(_DFLT_HTTP_MAX_HEADER_VAL_SIZ, http_conf.http_max_header_val_siz);
	_http_max_header_amt_siz = define_size(_DFLT_HTTP_MAX_HEADER_VAL_SIZ, http_conf.http_max_header_amt_siz);
	_http_max_body_siz = define_size(_DFLT_HTTP_MAX_BODY_SIZ, http_conf.http_max_body_siz);
}
...
redispatch:
		switch (_state)
		{
			case S_PARS_START_CR:
				if (IS_CR(c))								{ SET_STATE(S_PARS_START_LF);										}
				else										{ GOTO(S_PARS_METHOD);												}
				break;

			case S_PARS_START_LF:					
				if (IS_LF(c))								{ SET_STATE(S_PARS_START_CR);										}
				else										{ SET_ERROR;														}
				break;

			case S_PARS_METHOD:
				if (IS_SP(c))								{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_URI);			}
				else if (IS_WSP(c) || IS_CRLF(c))			{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_NOT_IMPLEMENTED);							}
				break;

			case S_PARS_URI:
				if (IS_SP(c))								{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_HTTP_NAME);		}
				else if (IS_WSP(c) || IS_CRLF(c))			{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_URI_TOO_LONG); 								}
				break;

			case S_PARS_HTTP_NAME:
				if (c == '/')								{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_HTTP_MAJOR);		}
				else if (IS_WSP(c) || IS_CRLF(c))			{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_IM_A_TEAPOT); 								}
				break;

			case S_PARS_HTTP_MAJOR:
				if (c == '.')								{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_HTTP_MINOR);		}
				else if (IS_WSP(c) || IS_CRLF(c))			{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_IM_A_TEAPOT); 								}
				break;

			case S_PARS_HTTP_MINOR:
				if (IS_CR(c))								{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_HTTP_MINOR_LF);	}
				else if (IS_WSP(c) || IS_LF(c))				{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_IM_A_TEAPOT); 								}
				break;

			case S_PARS_HTTP_MINOR_LF:
				if (IS_LF(c))								{ SET_STATE(S_PARS_HEADER_KEY);										}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;


			case S_PARS_HEADER_KEY:
				if (c == ':' && _rcvd_header_key_bytes)		{ SET_STATE(S_PARS_HEADER_OWS_PRE);									}
				else if (IS_CR(c)) 							{ SET_STATE(S_PARS_ENDOF_HEADER_LF);								}
				else if (!IS_TCHAR(c))						{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE);					}
				break;

			case S_PARS_HEADER_OWS_PRE:
				if (IS_WSP(c))								{ continue;															}
				else if (IS_CR(c))							{ _commit_token(request); SET_STATE(S_PARS_ENDOF_HEADER_LF);		}
				else if (IS_VCHAR(c))						{ _commit_token(request); ; GOTO(S_PARS_HEADER_VALUE);				}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_HEADER_VALUE:
				if (IS_DQUOTE(c))							{ _in_dquote = !_in_dquote;											}

				if (IS_CR(c) && !_in_dquote)				{ SET_STATE(S_PARS_ENDOF_HEADER_LF);								}
				else if (IS_WSP(c) && !_in_dquote)			{ SET_STATE(S_PARS_HEADER_OWS_POST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE);					}
				break ;

			case S_PARS_HEADER_OWS_POST:
				if (IS_WSP(c))								{ continue;															}
				else if (IS_VCHAR(c) || c == ',')			{ GOTO(S_PARS_HEADER_OWS_PRE);										}
				else if (IS_CR(c))							{ SET_STATE(S_PARS_ENDOF_HEADER_LF);								}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENDOF_HEADER_LF:
				if (!IS_LF(c))								{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_commit_token(request)))	{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE);					}
				else										{ SET_STATE(S_PARS_BLANK_LINE_CR);									}
				break;

			
			case S_PARS_BLANK_LINE_CR:
				if (IS_CR(c))								{ SET_STATE(S_PARS_BLANK_LINE_LF);									}
				else if (IS_WSP(c) || IS_LF(c))				{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else										{ GOTO(S_PARS_HEADER_KEY);											}
				break;

			case S_PARS_BLANK_LINE_LF:
				if (IS_LF(c))								{ GOTO(S_PARS_INSPECT_REQUEST); 									}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}		
				break;
			...
			case S_PARS_INSPECT_REQUEST:
				...
				if (IS_STATE(S_PARS_HEAD_DONE))				{ ++i; GOTO(S_PARS_HEAD_DONE);										}
				else if (IS_STATE(S_PARS_ERROR))			{ GOTO_ERR;															}
				else										{ continue;															}
			...
			case S_PARS_RECV_BODY:
				...
				if (_rcvd_bytes == _expected_bytes)			{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_BODY_DONE);		}
				break;
			...
			case S_PARS_ENCD_SIZE:
				if (IS_HEXDIG(c))							{ _append_byte(c);													}
				else if (IS_CR(c))							{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_ENCD_SIZE_LF);	}
				else if (c == ';')							{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_ENCD_EXT_START);	}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_EXT_START:
				if (IS_TCHAR(c))							{ GOTO(S_PARS_ENCD_EXT_KEY);										}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_EXT_KEY:
				if (c == '=' && _rcvd_header_key_bytes)		{ SET_STATE(S_PARS_ENCD_EXT_VAL);									}
				else if (!IS_TCHAR(c))						{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE); 					}
				break;

			case S_PARS_ENCD_EXT_VAL:
				if(c == ';')								{ _commit_token(request); SET_STATE(S_PARS_ENCD_EXT_KEY);			}
				else if (IS_CR(c))							{ _commit_token(request); SET_STATE(S_PARS_ENCD_EXT_LF);			}
				else if (!IS_VCHAR(c))						{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE); 					}
				break;

			case S_PARS_ENCD_EXT_LF:
			case S_PARS_ENCD_SIZE_LF:
				if (IS_LF(c))								{ GOTO(S_PARS_ENCD_ASSESS_BODY_SIZE);								}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_ASSESS_BODY_SIZE:
				if (_expected_bytes)						{ SET_STATE(S_PARS_ENCD_BODY);										}
				else										{ SET_STATE(S_PARS_ENCD_TRAIL_START);								}
				break;

			case S_PARS_ENCD_BODY:
				_append_byte(c);

				if (_rcvd_bytes == _expected_bytes)			{ _commit_token(request); IFN_ERR_SET_STATE(S_PARS_ENCD_BODY_CR);	}
				break;

			case S_PARS_ENCD_BODY_CR:
				if (IS_CR(c))								{ SET_STATE(S_PARS_ENCD_BODY_LF);									}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_BODY_LF:
				if (IS_LF(c))								{ SET_STATE(S_PARS_ENCD_SIZE); 										}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_TRAIL_START:
				if (IS_CR(c))								{ GOTO(S_PARS_ENCD_END_CR);											}
				else if (IS_TCHAR(c))						{ GOTO(S_PARS_ENCD_TRAIL_KEY);										}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_TRAIL_KEY:
				if (c == ':' && _rcvd_header_key_bytes)		{ SET_STATE(S_PARS_ENCD_TRAIL_VAL);									}
				else if (!IS_TCHAR(c))						{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE); 					}
				break;

			case S_PARS_ENCD_TRAIL_VAL:
				if (IS_CR(c))								{ SET_STATE(S_PARS_ENCD_TRAIL_LF);									}
				else if (!IS_VCHAR(c))						{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				else if (IS_ERR(_append_byte(c)))			{ REJECT_REQUEST(E_HTTP_HEADER_FIELDS_TOO_LARGE); 					}
				break;

			case S_PARS_ENCD_TRAIL_LF:
				if (IS_LF(c))								{ _commit_token(request); SET_STATE(S_PARS_ENCD_TRAIL_START);		}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_END_CR:
				if (IS_CR(c))								{ SET_STATE(S_PARS_ENCD_END_LF);									}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;

			case S_PARS_ENCD_END_LF:
				if (IS_LF(c))								{ SET_STATE(S_PARS_BODY_DONE);										}
				else										{ REJECT_REQUEST(E_HTTP_BAD_REQUEST);								}
				break;
...
inline parse_state_t	HTTP_Parser::_commit_token(HTTP_Request& request)
{
	switch (_state)
	{
		...
		case S_PARS_ENDOF_HEADER_LF:
			...
			std::transform(_token_key.begin(),
							_token_key.end(),
							_token_key.begin(),
							::tolower);
		...
		case S_PARS_ENCD_SIZE:
			...
			if ( (_total_expected_bytes += _expected_bytes) > _http_max_body_siz) {
				request.status = E_HTTP_CONTENT_TOO_LARGE;
				SET_ERROR;
			}
			break;
...
inline parse_state_t	HTTP_Parser::_inspect_request(HTTP_Request& request)
{
	...
	{
		if (request.start_line.version.name != "HTTP" || major != 1 || minor != 1)
		{
			request.status = E_HTTP_BAD_REQUEST;
			return SET_ERROR;
		}
...

	{
		...
		for ( ; cur_header != last_header; ++cur_header) {
			...
			if (cur_header->first == "host")				{ ++host_count;		};
			if (cur_header->first == "content-length")		{ ++length_count;	};
			if (cur_header->first == "transfer-encoding")	{ ++encode_count;	};

			if (cur_header->second.find(',') != std::string::npos) {
				const http_header_t*	header = http_header(cur_header->first.c_str());
...

	{
		...
		if (host_count == 0) {			
			bool	host_count_compliance = (major < 1 || (major == 1 && minor == 0));
	...
	{
		if (length_count) {
			HTTP_Request::header_it_t	hd = request.headers.find("content-length");
			...
			if (!char_tools::all_of(hd->second, ::isdigit)) {
				...
				if (errno == ERANGE || request.entity_body.size > _http_max_body_siz) {
					request.status = E_HTTP_CONTENT_TOO_LARGE;
					return SET_ERROR;
				}
				else if (*endptr != '\0') {
