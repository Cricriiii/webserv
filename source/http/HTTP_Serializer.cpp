...
response_status_t	HTTP_Serializer::serialize_head(const HTTP_Response& response, resource_context_t& resource, char* target, size_t target_size)
{
...
redispatch:
	switch (_state) {
		...
		case SEND_START_LINE:
			...
			std::copy(_buffer.begin() + _pos, _buffer.begin() + _pos + to_copy, target + total_copied);
...
response_status_t	HTTP_Serializer::serialize_body(const HTTP_Response& response, resource_context_t& resource, char* target, size_t target_size)
{
	(void) resource;
...
redispatch:
	switch (_state) {
		...
		case BUILD_CGI_BODY:
			...
			std::copy(response.entity_body.data.begin() + _pos, response.entity_body.data.begin() + _pos + to_copy, target + total_copied);
			total_copied += to_copy;
			_pos += total_copied;
